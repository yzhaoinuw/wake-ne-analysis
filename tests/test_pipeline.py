...
from wake_ne_analysis.expanded_features import (
    FEATURE_COLUMNS as EXPANDED_FEATURE_COLUMNS,
...
def test_expanded_feature_archives_store_29_features_and_burst_metadata(tmp_path):
    ...
    assert len(EXPANDED_FEATURE_COLUMNS) == 29
    assert features.shape == (seconds, 33)  # recording/second/label, 29 features, and state
    ...
    output_dir = tmp_path / "features_29"
    archives = write_expanded_feature_archives(tmp_path, output_dir)
    assert [archive.name for archive in archives] == ["expanded.npz"]
    with np.load(archives[0], allow_pickle=False) as archive:
        assert archive["X"].shape == (seconds, 29)
        assert tuple(archive["feature_names"].tolist()) == EXPANDED_FEATURE_COLUMNS
        assert archive["scaling_used_standard_deviation"].shape == (29,)
