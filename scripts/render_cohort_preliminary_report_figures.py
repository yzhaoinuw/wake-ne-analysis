...
STATE_COLORS = {"active_wake": "#1769aa", "quiet_wake": "#d95f02"}
METRICS = [
...
def coverage_figure(coverage: pd.DataFrame):
    ...
    for column, state in enumerate(("active_wake", "quiet_wake"), start=1):
        for seconds, color in ((15, "#1769aa"), (20, "#8fb9d9")):
    ...
    figure.update_layout(
        barmode="group",
        template="plotly_white",
        title="Why 15 seconds is the common spectral fallback",
...
def peak_assignment_figure(events: pd.DataFrame, mat_path: Path):
    import plotly.graph_objects as go

    chosen = events.loc[
        (events.recording_id == "unverified_35_updated")
        & (events.state == "quiet_wake")
        & events.complete
        & events.crosses_state
        & ~events.recording_start_qc_excluded
    ].iloc[0]
    recording = load_recording(mat_path, "unverified_35", "unverified_35_updated")
    left, right = float(chosen.onset20_seconds - 20), float(chosen.offset20_seconds + 20)
    ...
    figure = go.Figure()
    fills = {"active_wake": "rgba(23,105,170,0.18)", "quiet_wake": "rgba(217,95,2,0.18)", "other": "rgba(150,150,150,0.18)"}
    for start, stop, state in _state_spans(recording, left, right):
        figure.add_vrect(x0=start, x1=stop, fillcolor=fills[state], line_width=0, layer="below")
    ...
    figure.update_layout(
        template="plotly_white",
        title="Peak-state attribution: the whole complete episode is assigned to the state at its peak",
    ...
    figure.add_annotation(
        ...
        xref="paper",
        yref="paper",
        text="Blue = Active Wake; orange = Quiet Wake; gray = other state. Purple lines show the crossings used for duration and 20–80% slopes.",
        showarrow=False,
...
def main(argv=None):
    ...
    _save(peak_assignment_figure(events, args.mat), args.output / "peak_state_assignment_example.png", 600)
