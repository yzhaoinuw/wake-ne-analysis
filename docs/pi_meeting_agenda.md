...
|---:|---|---|---|
| 1 | When an NE transient spans a label transition, should it be included only if fully contained in one state, or assigned by its peak state? | Fully contained for the primary estimate, because its 20%-to-20% support is pure-state. Peak-state assignment can be a labelled sensitivity analysis. | 15/35 Active and 21/25 Quiet candidates cross a boundary; the choice materially changes the usable event set. |
